    ...
    with
        member this.GetDelta ()
            = match this with
              ...
              | Left  -> (-1, 0)
              | Right -> (+1, 0)
type State = State of Rope * ((int * int) Set)
    with
        static member emptyState (ropeSize: int): State = State (List.init ropeSize (fun _ -> (0, 0)), Set.singleton (0, 0))
...
let moveRope (moveDir: Direction) (State ((headX, headY) :: rem, visited)): State = 
    ...
    State (movedRope, visited.Add (List.last movedRope))
...
let solve (ropeSize: int) = Seq.map (parseCommand) >> Seq.fold (fun acc f -> f acc) (State.emptyState ropeSize) >> (fun (State (_, positions)) -> positions.Count) >> string 
